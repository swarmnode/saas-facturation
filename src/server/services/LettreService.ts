...
export class LettreService {
  ...
  static async getNextLettre(entreprise_id: number, txClient?: any): Promise<string> {
  ...
  static async lettrerPaiement(facture_id: number, entreprise_id: number, txClient?: any): Promise<string | null> {
    const q = txClient ? txClient.query.bind(txClient) : query;
    const r = await q(
    ...
    const lettre = await this.getNextLettre(entreprise_id, txClient);
    await q(
  ...
  static async lettrerAvoir(avoir_id: number, facture_origine_id: number, entreprise_id: number, txClient?: any): Promise<string | null> {
    const q = txClient ? txClient.query.bind(txClient) : query;
    const r = await q(
